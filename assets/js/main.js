...
(function() {
  ...
  function initCursorGlow() {
    ...
    const trailCount = 8;
    const trailDots = [];
    const trailPositions = Array.from({ length: trailCount }, () => ({ x: window.innerWidth / 2, y: window.innerHeight / 2 }));
    ...
    document.addEventListener('mousemove', (event) => {
      ...
      if (event.target.closest('a, button, input, textarea, .btn-getstarted, .navmenu a')) {
    ...
    function animateTrail() {
      ...
      trailPositions.forEach((pos, index) => {
        ...
        const intensity = 1 - index / trailCount;
        dot.style.left = `${pos.x}px`;
        dot.style.top = `${pos.y}px`;
        dot.style.opacity = `${0.24 * intensity}`;
        dot.style.transform = `translate(-50%, -50%) scale(${0.7 + 0.2 * intensity})`;
      });
